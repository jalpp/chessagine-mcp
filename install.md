...
ChessAgine MCP works with zero configuration, but four tools/tool-groups support personal API keys for services beyond Stockfish/ChessDB, which are all key-free: Lichess explorer/games/studies tools, ChessBoard Magic tools, and ChessDojo PAT tools. **How you supply a key depends on whether you're running the local stdio server or using the hosted remote server** — the two behave differently on purpose.
  ...
  "mcpServers": {
    "chessagine-mcp": {
      ...
      "env": {
        ...
        "DOJO_PAT_TOKEN": "your-chessdojo-pat"
      }
    }
  }
}
...
The hosted server at `https://chessagine-mcp.vercel.app/mcp` (and any Vercel/Express deployment built from `api/mcp.ts` / `src/runner/remote.ts`) intentionally has **no server-side credentials of its own**. Since it's a single shared deployment used by many people at once, there's no safe "server default" to fall back to; every caller supplies their own. Pass the token via following headers
...
For LibreChat's `librechat.yaml`, add the same entries under `headers:` on the server, e.g.:
...
5. Optionally fill in the Lichess/ChessBoard-Magic/ChessDojo API keys in the extension's config panel (all optional).
...
- **Lichess/ChessBoardMagic/ChessDojo tools return unauthenticated/rate-limited results (or auth errors) on the remote server** — the hosted server has no credentials of its own; set `X-Lichess-Token` / `X-Chessboardmagic-Token` / `X-Dojo-Token` as static headers in your client's server config (see [API keys / optional credentials](#api-keys--optional-credentials)), or pass a `token` argument in conversation if your client can't set custom headers.
- **ChessDojo tools point at the wrong deployment** — set the `DOJO_ENDPOINT` env var (local stdio) to your own `/pat/*` API Gateway URL if you're not using the reference deployment.
